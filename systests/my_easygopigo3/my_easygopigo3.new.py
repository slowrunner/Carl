...
"""
  ...
  - EasyGoPiGo3(): added noinit option with False default
    (added for IMU and RPI-Monitor vBatt)

  - steer(): modified to work, (use self.speed instead of NO_LIMIT_SPEED)

  - env: changed to python3

  - imports my_gopigo3 for noinit features in IMU and EasyGoPiGo3
...
# Set the following to True to enable debug print statements to appear
_DEBUG = False
def debug(in_str):
    if _DEBUG:
        print(in_str)
...
class EasyGoPiGo3(gopigo3.GoPiGo3):
    ...
    def drive_cm(self, dist, blocking=True, timeout=60):
        ...
        :param float timeout = 60: (blocking is True) Will stop driving and force return when target not reached in timeout seconds
        ...
        # save the current time (in float seconds since epoch)
        startTime = time.time()
    ...
    def drive_inches(self, dist, blocking=True, timeout=60):
        ...
        """
        self.drive_cm(dist * 2.54, blocking, timeout)

    def drive_degrees(self, degrees, blocking=True, timeout = 60):
        """
        ...
        if blocking:
            while self.target_reached(
                ...
                if (time.time() - startTime) > timeout:
                    self.stop()
                    debug("drive_degrees() timeout occured")
                    break
        return
    ...
    def orbit(self, degrees, radius_cm=0, blocking=True, timeout=60):
        ...
        :param boolean blocking = True: Set it as a blocking or non-blocking method.
        :param float timeout = 60: (blocking is True) Will stop orbit and force return when target not reached in timeout seconds
    ...
    def reset_encoders(self, blocking=True, timeout=60):
        ...
        :param float timeout = 60: (blocking is True) Will stop turning and force return when target not reached in timeout seconds
        ...
        if blocking:
            ...
            while motor_left_previous != self.MOTOR_LEFT or motor_right_previous != self.MOTOR_RIGHT:
                ...
                if (time.time() - startTime) > timeout:
                    self.stop()
                    debug("reset_encoders() timeout occured")
                    break
    ...
    def turn_degrees(self, degrees, blocking=True, timeout=60):
        ...
        if blocking:
            while self.target_reached(
                ...
                if (time.time() - startTime) > timeout:
                    self.stop()
                    debug("turn_degrees() timeout occured")
                    break
