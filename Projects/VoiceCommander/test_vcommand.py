...
def getVoiceCommand(model=model, rate=vosk_rate, commands=cmd_keywords, printResults=False, timeout=60):
	...
	try:
		...
		rec = KaldiRecognizer(model, 48000, commands)
		...
		stream = p.open(format=pyaudio.paInt16, channels=1, rate=48000, input=True, frames_per_buffer=8000, input_device_index = 2)
		print("test_vcommand: p.open success")
		stream.start_stream()
		print("test_vcommand: stream start success")
