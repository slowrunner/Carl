...
"""
    ...
    Target wall following path is 7.5 inches from wall
    to allow clearance behind the robot to turn away from wall 90 or 180 degrees


    Turning circle is 6.5 inches. Allowing an extra 1 inch for baseboards.
    ...
    Stopping Conditions (Diagonal Wall Following Distance = 10.6 inches):
    - No Wall: Distance reading > 1.5 x Diagonal Distance (approx 16 inches)
    - No Wall: Distance reading jumps up by 20% of Diagonal Distance (approx 2 inches)
...
def follow_wall(egpg,right0_left1=0):
    ...
    logging.info("distance reading: {:.1f}".format(distance_reading))
    ...
    while (LOST_WALL_DISTANCE > distance_reading > SAFE_FOLLOW_DISTANCE):
        ...
        logging.info("distance reading: {:.1f} inches,  steering value: {:.2f}".format(distance_reading,steering))
        time.sleep(0.05)
