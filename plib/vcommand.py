...
def getVoiceCommand(model=model, rate=vosk_rate, commands=cmd_keywords, printResults=False, timeout=60):
	...
	try:
		...
		p = pyaudio.PyAudio()
		# Carl needs to use input_device_index 25 mic with PiOS Feb 2021 - device 25 handles rate conversions
		stream = p.open(format=pyaudio.paInt16, channels=1, rate=16000, input=True, frames_per_buffer=8000, input_device_index = 25)
		stream.start_stream()
