...
"""Based on https://github.com/Pankaj-Baranwal/pocketsphinx  (/misc)"""

""" This program will launch a subprocess with the command:
         rec -q -c 1 -r 16000 -b 16 testing_audio.wav
    to record the user speaking prompted words from the keyword list,
    and a few non-key words with the prompt: [RANDOM]

    The user should press the return key after speaking each prompted word

    The program will kill the rec process, and then use the audio to
    test recognition of each key word against the key word list varying
    the threshold to achieve the optimum correct and minimum false accepts
    possible.

    If threshold for a word ends up near 1e-49 [...,49,...]
    the pronunciation for that keyword should be compared to the 
    CMU Pronounciation Dictionary at http://www.speech.cs.cmu.edu/cgi-bin/cmudict
"""
...
# en-us model location (not en-us/en-us on my installation)
    ...
    while 0 in processed:
        ...
        for i, val in enumerate([_fa, _missed][_z == 0]):
          try:
            ...
            print("val[1]:",val[1]," [_previous_fa[{}][1], _previous_missed][_z == 0]:{}".format(i,[_previous_fa[i][1], _previous_missed][_z==0]))
...
#    DIC_FILE = "..//demo/voice_cmd.dic"
#    KWLIST_FILE = "../demo/automated.kwlist"
    if len(sys.argv) == 3:
        DIC_FILE = sys.argv[1]
        KWLIST_FILE = sys.argv[2]
        preprocess_files(DIC_FILE, KWLIST_FILE)
    else:
        print("USAGE: ./set_kws_threshold.py grams/xyz.dic grams/xyz.kwlist")
