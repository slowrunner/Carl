...
#
# USAGE:
# ./carlmotion.py -i ip.ip.ip.ip -o port [-v 8.2] [-f NN] (background frame set)
#
...
def detect_motion(frameCount, egpg, volts):
    ...
    while True:
        batt = egpg.volt()
        if (egpg != None) and (batt < volts):
            print("detect_motion detected low voltage {} < {}".format(batt,volts))
            time.sleep(5)
            exit(0)
...
if __name__ == '__main__':
    ...
    ap.add_argument("-v", "--volts", type=float, default=8.2,
        help="stop server if volts drop below X.X volts (8.2v)")
    ...
    t = threading.Thread(target=detect_motion, args=(
        args["frame_count"],egpg,float(args["volts"]),))
    t.daemon = True
    t.start()
    ...
    t.join()
    print("detect_motion completed")
    print("shutting down flask")
    app.do_teardown_appcontext()
    time.sleep(5)
    raise KeyboardInterrupt
    print("flask app shutdown")
